...
class Encoder(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, n_layers, dropout_rate):
        ...
        self.bigru = nn.GRU(
            embed_size, hidden_size, n_layers, bidirectional=True, dropout=dropout_rate
        )
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        gru_input = torch.cat(
            [embedded, context_vector], dim=2
        )  # (1, batch_size, embed_size + hidden_size*2)
        ...
        pred = self.fc(
            torch.cat([embedded, output, context_vector], dim=1)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, target, teacher_force_ratio=0.5):
        ...
        outputs = torch.zeros((target_seq_length, batch_size, target_vocab_size)).to(
            device
        )
...
def train_fn(model, dataloader, optimizer, criterion, clip=4.0):
    ...
    for idx, (src, tgt) in enumerate(
        tqdm.tqdm(dataloader, total=len(dataloader), position=0, leave=True)
    ):
        ...
        output = output.view(
            -1, output.shape[-1]
        )  # (target_seq_length*batch_size, output_size)
...
def eval_fn(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for src, tgt in tqdm(dataloader, total=len(dataloader), position=0, leave=True):
            ...
            output = model(
                src, tgt, teacher_force_ratio=0
            )  # turn off teacher force during evaluation
...
def greedy_decode(model, sentence, max_length=50):
    ...
    source = (
        torch.tensor(token_ids, dtype=torch.long).unsqueeze(1).to(device)
    )  # (seq_length, 1)
...
def beam_decode(model, sentence, max_length=50, beam_width=5):
    ...
    for _ in range(max_length):
        ...
        for i in range(len(topk)):
            ...
            )  # log prob, minus because of max-heap
    ...
    return (
        max(result_sequences, key=lambda x: x[0])[1]
        if result_sequences
        else heappop(queue)[1]
    )
