...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, n_layers=2, dropout_p=0.1):
        ...
        self.lstm = nn.LSTM(
            embed_size, hidden_size, n_layers, batch_first=True
        )  # batch_first for (batch_size, seq_length)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_output, encoder_hidden, encoder_cell, target_tensor=None):
        ...
        )  # concat output token (batch_size, seq_length, output_size)
        decoder_outputs = F.log_softmax(
            decoder_outputs, dim=-1
        )  # probs (batch_size, seq_length, output_size)
...
class EncoderBiRNN(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, n_layers=1, dropout_p=0.1):
        ...
        self.embedding = nn.Embedding(input_size, embed_size)
        self.gru = nn.GRU(
            embed_size, hidden_size, n_layers, batch_first=True, bidirectional=True
        )
    ...
    def forward(self, input):
        ...
        outputs, hidden = self.gru(
            embedded
        )  # output(batch_size, seq_length, hidden_size * 2) , hidden(n_layers * 2, batch_size, hidden_size)
        hidden_cat = torch.cat(
            [hidden[0, :, :], hidden[1, :, :]], dim=1
        )  # concat forward and backward final hidden state(batch_size, hidden_size * 2)
        hidden_out = self.fc(
            hidden_cat
        )  # initial decoder hidden state (batch_size, hidden_size)
...
class BahdanauAttention(nn.Module):
    def __init__(self, hidden_size):
        ...
        )  # encoder output is bidirectional
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        attn_weights = F.softmax(
            scores, dim=-1
        )  # attention weights (batch_size, seq_length)
        context = torch.bmm(
...
class AttnDecoder(nn.Module):
    def __init__(self, output_size, embed_size, hidden_size, n_layers, dropout_p=0.5):
        ...
        self.gru = nn.GRU(
            ...
            batch_first=True,
            bidirectional=False,
        )  # input embedded + context vector
        self.fc = nn.Linear(
            embed_size + hidden_size + hidden_size * 2, output_size
        )  # input embedded + gru output + context vector
        self.dropout = nn.Dropout(dropout_p)
    ...
    def forward(self, encoder_outputs, encoder_hidden, target_tensor=None):
        ...
        decoder_input = torch.empty(batch_size, 1, dtype=torch.long).fill_(
            SOS_TOKEN
        )  # first start token (batch_size, 1)
        decoder_hidden = encoder_hidden
        decoder_outputs = []
        attentions = []
        ...
        for i in range(MAX_LENGTH):
            ...
            if target_tensor is not None:
                ...
                decoder_input = decoder_output.argmax(dim=-1, keepdim=True).squeeze(
                    -1
                )  # top 1
    ...
    def forward_step(self, input, hidden, encoder_outputs):
        ...
        output = self.fc(
            torch.cat([embedded, output_gru, context], dim=2)
