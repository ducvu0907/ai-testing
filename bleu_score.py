...
def compute_precision(i_gram: int, candidate: str, references: list[str]):
    ...
    for n_gram in candidate_counter:
        max_count_in_refs = max(
            ref_counter[n_gram] for ref_counter in reference_counters
        )
...
def compute_bleu_score(candidate: str, references: list[str]):
    ...
    brevity_penalty = min(
        1, math.exp(1 - reference_length / output_length)
    )  # accounts for the short length candidate
    ps_n = []
    for i in range(1, 5):
